...
interface AppContextType {
  ...
  addAgent: (agentData: Omit<AdminAgent, 'id' | 'rating' | 'propertiesCount'>) => void;
  updateAgentStatus: (id: string, status: 'Active' | 'Pending Approval' | 'Suspended') => void;
  deleteAgent: (id: string) => void;
  updateAdminProfile: (data: { name: string; email: string; phone?: string }) => void;
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchNextPropertiesPage = useCallback(async () => {
    ...
    try {
      const res = await apiService.getProperties({ page: nextPageToFetch, limit: 100 });
  ...
  const [adminAgents, setAdminAgents] = useState<AdminAgent[]>([
    { id: 'agt-1', name: 'Karan Kang', email: 'realtorkarankang@gmail.com', phone: '+1 (437) 998-5873', photo: '/karan-kang.jpg', experience: '12 Years', rating: 4.99, propertiesCount: 18, status: 'Active', commissionRate: '2.5%' },
    { id: 'agt-2', name: 'Elena Rostova', email: 'elena@novaestate.ca', phone: '+1 (416) 555-0199', photo: 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?auto=format&fit=crop&w=200&q=80', experience: '18 Years', rating: 4.98, propertiesCount: 12, status: 'Active', commissionRate: '2.5%' },
    { id: 'agt-3', name: 'Sébastien LeClerc', email: 'sebastien@novaestate.ca', phone: '+1 (514) 555-0188', photo: 'https://images.unsplash.com/photo-1560250097-0b93528c311a?auto=format&fit=crop&w=200&q=80', experience: '14 Years', rating: 4.95, propertiesCount: 9, status: 'Active', commissionRate: '2.5%' },
    { id: 'agt-4', name: 'Victoria Lin', email: 'victoria@novaestate.ca', phone: '+1 (604) 555-0166', photo: 'https://images.unsplash.com/photo-1580489944761-15a19d654956?auto=format&fit=crop&w=200&q=80', experience: '10 Years', rating: 4.92, propertiesCount: 8, status: 'Active', commissionRate: '2.0%' }
  ]);
  ...
  const addAgent = (agentData: Omit<AdminAgent, 'id' | 'rating' | 'propertiesCount'>) => {
    ...
    setAdminAgents(prev => [newAgent, ...prev]);
    showToast(`Agent ${agentData.name} onboarded successfully!`, 'success');
  };
  ...
  const updateAdminProfile = (data: { name: string; email: string; phone?: string }) => {
    setUser(prev => prev ? { ...prev, name: data.name, email: data.email } : { name: data.name, email: data.email, role: 'admin' });
    showToast('Admin credentials & security profile updated successfully!', 'success');
  };
